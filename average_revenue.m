...
% Boucle sur chaque scénario pour tracer les revenus moyens
for i = 1:length(scenarios)
    ...
    % Calcul des revenus moyens par année
    revenue_market = zeros(length(years), 1);
    revenue_tariff = zeros(length(years), 1);
    
    for j = 1:length(years)
        idx = data.year == years(j);
        revenue_market(j) = mean(data.Pd(idx) .* data.base_price(idx));
        revenue_tariff(j) = mean(data.Pd(idx) .*data.tariff(idx));
    end
    ...
    title([scenarios{i}]);
    xlabel('Year');
    ylabel('Average revenue per hour');
...
%sgtitle('Revenus Moyens du Stockage par Année et Scénario');
