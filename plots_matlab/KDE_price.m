...
LOAD_FILE = fullfile(DATA_PATH, 'day_ahead_prices.csv');
...
xlabel('Day-ahead price (€/MWh)', 'FontSize', 12, 'FontWeight', 'bold');
...
text(min_val, max(f) * 0.1, sprintf('Min: %.2f €/MWh', min_val), 'Color', 'black', 'FontSize', 10);
text(max_val, max(f) * 0.1, sprintf('Max: %.2f €/MWh', max_val), 'Color', 'black', 'FontSize', 10);
